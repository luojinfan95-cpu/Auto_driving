...
def is_valid_action(output):
    return (
        isinstance(output, list) and
        len(output) == 6 and
        all(
            isinstance(t, tuple) and len(t) == 2 and
            all(isinstance(x, (int, float)) for x in t)
            for t in output
        )
    )

def extract_driving_action(pred_action_str, error_handling):
    if not error_handling:
        try:
            pred_actions = ast.literal_eval(pred_action_str)
            if is_valid_action(pred_actions):
                return pred_actions
            else:
                return None
        except:
    ...
    numbers = []
    i = 0
    while i < len(pred_action_str):
        # Skip characters that are not part of a number
        while i < len(pred_action_str) and not (pred_action_str[i].isdigit() or pred_action_str[i] in '-.'):
            i += 1
        start = i
        # Collect characters that could be part of a number
        while i < len(pred_action_str) and (pred_action_str[i].isdigit() or pred_action_str[i] in '-.'):
            i += 1
        candidate = pred_action_str[start:i]

        # Match a valid float with optional minus sign
    ...
    return [(numbers[i], numbers[i+1]) for i in range(0, 12, 2)]
...
def compute_metrics(predicted_trajectory, ground_truth_trajectory):
    ...
    ADE_1s, ADE_2s, ADE_3s, ADE_avg, FDE, and missRate_2
    ...
    # For ADE_1s (1 second horizon - first 2 points)
    if len(pred_traj) >= 2 and len(gt_traj) >= 2:
        gt_pos_1s = gt_traj[:2]
        pred_traj_1s = pred_traj[:2]
        ade_per_timestep_1s = np.linalg.norm(gt_pos_1s - pred_traj_1s, axis=1)
        metrics["ADE_1s"] = np.mean(ade_per_timestep_1s)
    
    # For ADE_2s (2 second horizon - middle 2 points)
    if len(pred_traj) >= 4 and len(gt_traj) >= 4:
        gt_pos_2s = gt_traj[2:4]
        pred_traj_2s = pred_traj[2:4]
        ade_per_timestep_2s = np.linalg.norm(gt_pos_2s - pred_traj_2s, axis=1)
        metrics["ADE_2s"] = np.mean(ade_per_timestep_2s)
    
    # For ADE_3s (3 second horizon - last 2 points)
    if len(pred_traj) >= 6 and len(gt_traj) >= 6:
        gt_pos_3s = gt_traj[4:6]
        pred_traj_3s = pred_traj[4:6]
        ade_per_timestep_3s = np.linalg.norm(gt_pos_3s - pred_traj_3s, axis=1)
        metrics["ADE_3s"] = np.mean(ade_per_timestep_3s)
